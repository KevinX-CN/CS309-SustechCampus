  ...
  <div class="container">
    <div class="main">
      ...
      <div class="loginbox">
          ...
          <br>
          <div class="log-box">
            <button type="primary" class="login_btn" @click="visitor"> 游客登录</button>
...
export default {
  ...
  methods:{
    ...
    visitor(){this.$router.push("model/demo")},
    ...
    login() {
      // 如果是有参数传递
      this.$axios.get(`http://localhost:8082/exer/login/${this.name}/${this.pwd}`)
        .then(res => {
          this.user_list = res.data;
          if (this.user_list.status !== 'notexit') {
            console.log(this.user_list.name);
            // 可以跳转到主页
            this.$router.push({
              path: "/model",
              query: {
                username: this.user_list.name,
                password: this.user_list.password,
                type: this.user_list.type,
                status: this.user_list.status
              }
            });
          } else {
