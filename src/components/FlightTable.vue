    ...
    <el-table :data="flights" style="width: 80%;;margin: 0 auto;">
      ...
      <el-table-column label="Operations">
        <template #default="scope">
          ...
          <el-button type="info" @click="editFlight(scope.$index)">Edit</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    createNewFight() {
      ...
      this.FlightForm = { ...this.initialFlightForm }
    ...
    editFlight(index) {
      ...
      this.FlightForm = {
        FlightNo: selectedFlight['Flight No'],
        AirlineCompany: selectedFlight['Airline Company'],
        From: selectedFlight.From,
        To: selectedFlight.To,
        Date: selectedFlight.Date,
        DepartureTime: selectedFlight['Departure Time'],
        ArriveTime: selectedFlight['Arrival Time']
      };
      this.dialogVisible = true;
    },
    AddFlight(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.selectedFlightIndex !== -1) {
            // 在编辑模式下更新选中航班的值
            const selectedFlight = this.flights[this.selectedFlightIndex];
            selectedFlight['Flight No'] = this.FlightForm.FlightNo;
            selectedFlight['Airline Company'] = this.FlightForm.AirlineCompany;
            selectedFlight.From = this.FlightForm.From;
            selectedFlight.To = this.FlightForm.To;
            selectedFlight.Date = this.FlightForm.Date;
            selectedFlight['Departure Time'] = this.FlightForm.DepartureTime;
            selectedFlight['Arrival Time'] = this.FlightForm.ArriveTime;
          } else {
            // 在添加模式下添加新的航班
            this.flights.push({
