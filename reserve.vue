        ...
        <el-row v-if="selectedBuilding">
          <el-col :span="6" v-for="(room, index) in selectedBuilding.rooms" :key="index">
            <div class="room-card">
              ...
              <div class="room-status" :class="{'room-available': room.available}">
                {{ room.available ? '可预订' : '已预订' }}
              </div>
              <el-button
                v-if="room.available"
              ...
              <el-form ref="bookingForm" :model="bookingForm" label-width="100px">
                ...
                <el-form-item label="时间段">
                  <el-select v-model="bookingForm.timeSlot" placeholder="选择时间段">
                    <el-option
                      v-for="timeSlot in room.availableTimes"
                      :key="timeSlot"
      ...
      <div v-if="bookings.length > 0">
        ...
        <ul>
          <li v-for="(booking, index) in bookings" :key="index">
            {{booking.building}}-{{booking.room}}-{{ booking.date }} {{ formatTimeSlot(booking.timeSlot) }}
...
.building{
  background:url('../assets/web.jpeg');
  width:100%;
  height:100%;
  position:fixed;
